...
mod decompressor_tests {
    ...
    fn test_decompress_stream() {
        ...
        let result = decompressor
            .decompress(&mut reader, &mut writer, None)
            .unwrap();
    ...
    fn test_decompress_with_expected_size() {
        ...
        let result = decompressor
            .decompress(&mut reader, &mut writer, Some(data.len() as u64))
            .unwrap();
...
mod roundtrip_tests {
    ...
    fn roundtrip_verify(original: &[u8]) {
        ...
        decompressor
            .decompress(&mut reader, &mut decompressed, None)
            .unwrap();
    ...
    fn test_crc_mismatch_detection() {
        ...
        let result =
            decompressor.decompress_and_verify_crc32(&mut reader, &mut writer, wrong_crc, None);

        assert!(result.is_err(), "Should fail with wrong CRC");
    }

    #[test]
    fn test_roundtrip_lz77_huffman() {
        ...
        let mut compressed = Vec::new();
        let mut reader = Cursor::new(&original);
        let compress_result = compressor.compress(&mut reader, &mut compressed).unwrap();

        // Debería haber compresión real en datos repetitivos
        assert!(
            compress_result.compressed_size < compress_result.original_size,
        ...
        let mut reader = Cursor::new(&compressed);
        decompressor
            .decompress(&mut reader, &mut decompressed, Some(original.len() as u64))
            .unwrap();
...
mod archive_tests {
    ...
    #[test]
    fn test_zip_compress_decompress_roundtrip() {
        let temp = tempdir().unwrap();
        let file_path = temp.path().join("source.txt");
        let zip_path = temp.path().join("archive.zip");
        let dest_dir = temp.path().join("extract");

        // Crear archivo de origen
        let content = b"This is some text data to be zipped and unzipped!";
        let mut f = File::create(&file_path).unwrap();
        f.write_all(content).unwrap();

        // Comprimir a zip
        cszip::utils::archive::compress_zip(&file_path, &zip_path).unwrap();
        assert!(zip_path.exists());

        // Descomprimir el zip
        cszip::utils::archive::decompress_zip(&zip_path, &dest_dir).unwrap();

        let extracted_file = dest_dir.join("source.txt");
        assert!(extracted_file.exists());

        let mut f2 = File::open(&extracted_file).unwrap();
        let mut read_content = Vec::new();
        std::io::Read::read_to_end(&mut f2, &mut read_content).unwrap();
