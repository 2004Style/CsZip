...
mod compress_tests {
    ...
    fn test_compress_simple_file() {
        ...
        assert!(output.status.success(), "Compress should succeed: {:?}", 
            String::from_utf8_lossy(&output.stderr));
        
        // Verificar que se creó el archivo .cz
        let compressed = input.with_extension("txt.cz");
        assert!(compressed.exists(), "Compressed file should exist at {:?}", compressed);
    }
    ...
    fn test_compress_with_output_path() {
        ...
        assert!(output.status.success(), "Compress with output path should succeed: {:?}",
            String::from_utf8_lossy(&output.stderr));
    ...
    fn test_compress_with_force_overwrite() {
        ...
        assert!(!output_no_force.status.success(), "Should fail without force");
    ...
    fn test_compress_empty_file() {
        ...
        assert!(output.status.success(), "Empty file compression should succeed: {:?}",
            String::from_utf8_lossy(&output.stderr));
    }
    ...
    fn test_compress_large_content() {
        ...
        assert!(output.status.success(), "Large file compression should succeed: {:?}",
            String::from_utf8_lossy(&output.stderr));
    ...
    fn test_compress_with_level() {
        ...
        for level in [0, 5, 9] {
            ...
            assert!(output.status.success(), 
                ...
                level, String::from_utf8_lossy(&output.stderr));
        }
    }
}
...
mod decompress_tests {
    ...
    fn compress_helper(runner: &CliRunner, content: &[u8], name: &str) -> PathBuf {
        ...
        input.with_extension(format!("{}.cz", input.extension().unwrap_or_default().to_str().unwrap()))
    }
    ...
    fn test_decompress_simple() {
        ...
        assert!(decompress_result.status.success(), "Decompression failed: {:?}",
...
mod info_tests {
    ...
    fn test_info_basic() {
        ...
        assert!(stdout.contains("CZ") || stdout.contains("version") || stdout.len() > 0 || 
                output.stderr.len() > 0);
    }
...
mod verify_tests {
    ...
    fn test_verify_valid_file() {
        ...
        assert!(output.status.success(), "Verify should succeed for valid file: {:?}",
            String::from_utf8_lossy(&output.stderr));
    }
...
mod global_options_tests {
    ...
    fn test_help() {
        ...
        assert!(output.status.success());
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("cszip") || stdout.contains("CsZip") || stdout.contains("compress"));
    }
...
mod roundtrip_tests {
    ...
    fn test_roundtrip_multiple_files() {
        ...
        for (name, content) in &files_data {
            ...
            runner.run(&["compress", input.to_str().unwrap()]);
            let compressed = input.with_extension(format!("{}.cz", 
